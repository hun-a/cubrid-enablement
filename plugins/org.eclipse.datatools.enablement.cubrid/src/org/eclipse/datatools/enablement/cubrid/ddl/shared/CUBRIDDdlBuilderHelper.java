 ...
 *
 */
public abstract class CUBRIDDdlBuilderHelper {
...
//	protected final static String ENGINE = "ENGINE"; //$NON-NLS-1$
	...
	// add for CUBRID Trigger statement, 09-May-16
	protected final static String REJECT = "REJECT";	//$NON-NLS-1$
	protected final static String INVALIDATE_TRANSACTION = "INVALIDATE TRANSACTION";	//$NON-NLS-1$
	protected final static String PRINT = "PRINT";	//$NON-NLS-1$
	protected final static String EXECUTE = "EXECUTE";	//$NON-NLS-1$
...
//			if (isAutoInc(pk)) {
				/*
				 * Remove AUTO_INCREMENT before DROP PRIMARY KEY	
				 * This 'if clause' is used in MySQL
				 * but needless in CUBRID
				 */
//			}
			...
			String statement = ALTER 
							...
							+ TABLE 
							+ SPACE 
							+ getName(constraint.getBaseTable(), quoteIdentifiers, qualifyNames)
							+ SPACE 
							+ DROP 
							+ SPACE 
							+ PRIMARY_KEY;
			...
			String statement = ALTER
							...
							+ FOREIGN_KEY
							+ SPACE
							+ getName(constraint, quoteIdentifiers);
			statements.add(statement);
		}
	...
	public String addCheckConstraint(CheckConstraint constraint, boolean quoteIdentifiers, boolean qualifyNames) {
		return ALTER 
				...
				+ getName(constraint.getBaseTable(), quoteIdentifiers, qualifyNames) 
				+ SPACE
				+ this.getAddCheckConstraintClause(constraint, quoteIdentifiers);
	}
	...
	public String addForeignKey(ForeignKey foreignKey, boolean quoteIdentifiers, boolean qualifyNames) {
		...
		if (parentTable == null) {
			CUBRIDPlugin.getDefault().getLog().log(new Status(IStatus.ERROR,
					CUBRIDPlugin.ID, 0, 
	...
	protected String getColumnString(Column column, boolean quoteIdentifiers, boolean includeAutoInc) {
		...
		String columnString = columnName + SPACE + dataType;
		...
		if (defaultValue != null && defaultValue.trim().length() > 0) {
			...
			columnString += SPACE + DEFAULT + SPACE + defaultValue;
		}
		
		if (!column.isNullable()) {
			columnString += SPACE + NOT + SPACE + NULL;
		}
		if (includeAutoInc) {
		...
		if (containedType != null) {
			if (containedType instanceof PredefinedDataType) {
				...
				if (root instanceof Database) {
					DatabaseDefinition def = RDBCorePlugin.getDefault()
	...
	protected String getQuotedString(String original) {
		StringTokenizer tokenizer = new StringTokenizer(original, QUOTE);
		String result = null;
	...
	public abstract String createTable(BaseTable table, boolean quoteIdentifiers, 
			boolean qualifyNames, boolean generatePKConstraints);
	
	public abstract String createIndex(Index index, boolean quoteIdentifiers,
			boolean qualifyNames);
	
	public abstract String createTrigger(Trigger trigger, boolean quoteIdentifiers,
			boolean qualifyNames);
	
	public abstract String createView(ViewTable view, boolean quoteIdentifiers,
			boolean qualifyNames);
	
	public abstract String dropIndex(Index index, boolean quoteIdentifiers,
			boolean qualifyNames);
